...
def main():
    ...
    # Set vault secrets
    # vault = VaultLib([(DEFAULT_VAULT_ID_MATCH, VaultSecret('MyP@ssw0rd!'.encode()))])
    # vault = [(DEFAULT_VAULT_ID_MATCH, VaultSecret(''.encode('MyP@ssw0rd!')))]
    vault_password = open('../vault_password', 'r').read().strip()
    vault = [(C.DEFAULT_VAULT_ID_MATCH, VaultSecret(vault_password.encode()))]
    loader.set_vault_secrets(vault)

    # passwords = dict(vault_pass='MyP@ssw0rd!')
    ...
    play = Play().load(play_source, variable_manager=variable_manager, loader=loader, vars=None)
