...
createContainers() {
  ...
  for i in $(seq $id_min $id_max); do
    ...
    ${CONTAINER_CMD} ${CONTAINER_USER}-debian-$i /bin/sh -c "sed -i 's@\(.*stretch-backports.*\)@#\1@g' /etc/apt/sources.list"
  done
...
createAnsible() {
  ...
  sudo podman ps -aq | awk '{system("sudo podman inspect -f \"    {{.Name}}:\n      ansible_host: {{.NetworkSettings.IPAddress}}\" "$1)}' >>${ANSIBLE_DIR}/00_inventory.yml
  echo ""
}
